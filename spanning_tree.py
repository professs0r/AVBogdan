...
def func_percent_complete(graph, count_by_Kirchhoff):
    """
    ...
    for i in iterator:
        trees.append(i)
        counter += 1
        if float(counter/count_by_Kirchhoff) > loading:
            loading += 0.0001
            print("Процент выполнения:", "{0:0.2f}".format(counter / count_by_Kirchhoff * 100), " %.")
    return trees