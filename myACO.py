...
MAX_POINTS = 10  # максимальное количество точек для прохождения
# тестовая глобальная переменная для максимального количества муравьёв
MAX_ANTS = 200  # максимальное количество муравьёв
...
MAX_ITERATIONS = 100  # максимальное количество итераций, после которого алгоритм останавливается
    ...
    def func_probability_to_move(self, matrix_of_distance_after_division, pheromone_matrix):
        ...
        for stop in range(MAX_POINTS):
            if (self.visited_points[stop] == 0):
                multy_length_and_pheromone[stop] = pow(pheromone_matrix[start][stop], alpha) * pow(
                    matrix_of_distance_after_division[start][stop], beta)
